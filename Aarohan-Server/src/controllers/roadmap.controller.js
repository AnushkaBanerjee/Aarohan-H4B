...
const createRoadmap = asyncHandler(async (req, res) => {
    ...
    for (let i = 0; i < roadmap.length; i++) {
        const myRoadmap = await Roadmap.create({
            ...
            topic: subject,
            todo: roadmap[i].todo,
            names: roadmap[i].names,
            description: roadmap[i].description,
            index: i + 1
        })
    }
...
const generateRoadmap = asyncHandler(async (req, res) => {
    ...
    try {
        const roadmap = await axios.post(`${process.env.GENERATE_ROADMAP_API}/generate-roadmap?subject=${text}&studytimeDaily=${hour}&timeAvailable=${deadline}&semester=${semester}`);
...
const generateQuestions = asyncHandler(async (req, res) => {
    ...
    try {
        const questions = await axios.post(`${process.env.GENERATE_QUESTION_API}/generate-question?subject=${subject}&semester=${4}&subtopics=${names}`);
        return res.status(200).json(
            new ApiResponse(200, {
